...
public class ProcessVivoPerson extends IteratorMethods {
	...
	//public static final Resource NON_FACULTY_ACADEMIC = ResourceFactory.createResource("http://vivoweb.org/ontology/core#NonFacultyAcademicPosition");
	//public static final Resource NON_ACADEMIC_POSITION = ResourceFactory.createResource("http://vivoweb.org/ontology/core#NonAcademic");
	//public static final Resource FACULTY_ADMIN_POSITION = ResourceFactory.createResource("http://vivoweb.org/ontology/core#FacultyAdministrativePosition");
	...
	public String titleMapFile = MainClass.fileRDFPath + "jobtitles5.csv";
	...
	public CumulativeDeltaModeler processVivoperson(String personId, CumulativeDeltaModeler cdm) throws Exception {
		...
		if (mdlOnePersonVIVORDF.size() > 0 ) {
			...
			//this looks really redundant - is it the same?
			...
			//logger.debug(resultString);
				...
				String vivoPosnQuery = rw.ReadQueryString(MainClass.fileQryPath + "qStrVIVOPositionDesc.txt");
				...
				String hrisPosnQuery = rw.ReadQueryString(MainClass.fileQryPath + "qStrHRISPositionDesc.txt");
				...
				for (Statement stmt : vivoPosnList ) {
					...
					try {
						...
						mdlCorrectedHRISposnRdf.add(stmt.getSubject(),  RDF.type, positionType );
						Resource employeeType = chd.getEmployeeType(positionType);
						CorrectedHRISPersonRDF.add(stmt.getSubject(),  RDF.type, employeeType );
				...
				}  // end VIVO position list processing
				...
				for (Statement stmt1 : hrisPosnList ) {
					...
					try {
						...
						if (positionType != null) {
							...
							// employee type and primaryWorkingTitle go on the PERSON profile and need to be part of that diff
							// add primaryPosition here.
							if (headIndValue.equals("P")) {
								mdlCorrectedHRISposnRdf.add(vivoIndiv,  RDF.type, PRIMARY_POSITION );                                 
								Resource employeeType = chd.getEmployeeType(positionType);
								...
								//mdlOnePersonVIVORDF.add(vivoIndiv,  RDF.type, employeeType );
								mdlOnePersonVIVORDF.add(vivoIndiv, PRIMARY_WORKING_TITLE, prettyTitle);
				...
				} //end for stmt hris posn list processing
				...
				try {
					...
					logger.debug("**mdlVIVOPosnRDF before:\n");
					...
					logger.debug("**mdlVIVOPosnRDF after:\n");
					rw.LogRDF(mdlVIVOPosnRDF, "N3");
					...
					//String hrisPosnQuery = rw.ReadQueryString(MainClass.fileQryPath + "qStrGetPosnForOneHrisPerson.txt");
					String hrisOrgQuery = rw.ReadQueryString(MainClass.fileQryPath + "qStrGetPosnOrgForOneHrisPerson.txt");
					...
					if (!mdlHRISOrgRDF.isEmpty()) {
						...
						String hrisOrgVivoUriQuery = rw.ReadQueryString(MainClass.fileQryPath + "qStrGetHRISOrgRDFWithVivoUri.txt");
						String hrisOrgHrisUriQuery = rw.ReadQueryString(MainClass.fileQryPath + "qStrGetHRISOrgRDF.txt");		
				...
				// Pick this whole Try up and put it with position diff
				...
				try {
					...
					logger.info("ignoreDiffRetract." + ignoreDiffRetract);
					...
					logger.info("ignoreDiffAdd." + ignoreDiffAdd);
					if (ignoreDiffAdd) {
