...
public class CollectRdfForVivoPositionRetract extends IteratorMethods {
	...
	public CumulativeDeltaModeler getPositionRdfForRetract(String personId, CumulativeDeltaModeler cdm) throws Exception {
		...
		if (mdlOnePersonVIVORDF.size() > 0 ) {
			...
			logger.debug("Holy Cow!  You made it.  Now STOP.");			
			...
			//logger.debug(resultString);
				...
				String vivoPosnQuery = rw.ReadQueryString(MainClass.fileQryPath + "qStrVivoPosnForOnePerson.txt");
				...
				for (Statement stmt12 : vivoPositionList ) {
					...
					String vivoPosnRDFQuery = rw.ReadQueryString(MainClass.fileQryPath + "qStrVivoGetPositionRDF.txt");
				...
				for (Statement stmt : vivoPosnList ) {
					...
					String VivoOrgQuery = rw.ReadQueryString(MainClass.fileQryPath + "qStrGetPosnOrgForOneVivoPosition.txt");
				...
				try {
					...
					// get list of D2R positions for person
					String hrisPosnQuery = rw.ReadQueryString(MainClass.fileQryPath + "qStrHRISPosnForOnePerson.txt");
					...
					for (Statement stmt11 : hrisPosnList ) {
						...
						String hrisPosnRDFQuery = rw.ReadQueryString(MainClass.fileQryPath + "qStrHRISGetPositionRDF.txt");
